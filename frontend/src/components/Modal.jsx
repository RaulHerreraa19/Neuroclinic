...
export default function Modal({ open, onClose, title, subtitle, children, maxWidth = 'max-w-lg' }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div
        className={`relative w-full ${maxWidth} bg-white rounded-3xl shadow-2xl p-6 sm:p-8 animate-scale-in max-h-[90vh] overflow-y-auto`}
      >
        {(title || onClose) && (
          <div className="flex items-start justify-between gap-4 mb-5">
            <div>
              {title && <h3 className="text-xl font-bold text-navy-900">{title}</h3>}
            ...
            {onClose && (
              <button
                ...
                onClick={onClose}
                aria-label="Cerrar"
                className="w-9 h-9 flex-shrink-0 flex items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition"
              >
                ✕
              </button>
            )}
